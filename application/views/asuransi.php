				...
				<?php foreach ($asuransi as $asu) : ?>
					<tr>
						...
						<?php if ($asu['asuransi'] <= date('Y-m-d')) : ?>
							...
							<td><?php echo date('d M Y', strtotime($asu['asuransi'])) ?> <br> <span class="badge bg-danger">Telah Jatuh tempo <br> <?php echo round($jatuhtempo) ?> <br> Hari yang lalu</span></td>
						<?php else : ?>
							<?php $sisahari = (strtotime($asu['asuransi']) - strtotime(date('Y-m-d'))) / (60 * 60 * 24); ?>
							<?php if ($sisahari <= 7 && $sisahari > 1) : ?>
								<td><?php echo date('d M Y', strtotime($asu['asuransi'])) ?><br><span class="badge bg-danger"><?php echo round($sisahari); ?> Hari Lagi</span></td>
							<?php elseif ($sisahari <= 30) : ?>
								<td><?php echo date('d M Y', strtotime($asu['asuransi'])) ?> <br> <span class="badge bg-warning"><?php echo round($sisahari) ?> Hari Lagi</span></td>
							<?php else : ?>
								<td><?php echo date('d M Y', strtotime($asu['asuransi'])) ?> <br> <span class="badge bg-success"><?php echo round($sisahari) ?> Hari Lagi</span></td>
							<?php endif; ?>
						<?php endif; ?>
