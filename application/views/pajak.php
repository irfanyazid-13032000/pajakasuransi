				...
				<?php foreach ($pajak as $pajek) : ?>
					<tr>
						...
						<?php if ($pajek['stnk'] <= date('Y-m-d')) : ?>
							...
							<td><?php echo $pajek['stnk'] ?> <br> <span class="badge bg-danger">Telah Jatuh tempo <br> <?php echo round($jatuhtempo) ?> <br> Hari yang lalu</span></td>
						<?php else : ?>
							<?php $sisahari = (strtotime($pajek['stnk']) - strtotime(date('Y-m-d'))) / (60 * 60 * 24); ?>
							<?php if ($sisahari <= 7 && $sisahari > 1) : ?>
								<td><?php echo $pajek['stnk'] ?><br><span class="badge bg-danger"><?php echo round($sisahari); ?> Hari Lagi</span></td>
							<?php elseif ($sisahari <= 30) : ?>
								<td><?php echo $pajek['stnk'] ?> <br> <span class="badge bg-warning"><?php echo round($sisahari) ?> Hari Lagi</span></td>
							<?php else : ?>
								<td><?php echo $pajek['stnk'] ?> <br> <span class="badge bg-success"><?php echo round($sisahari) ?> Hari Lagi</span></td>
							<?php endif; ?>
						<?php endif; ?>
						<td>
